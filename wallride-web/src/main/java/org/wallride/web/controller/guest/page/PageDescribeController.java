...
public class PageDescribeController extends AbstractController {
	...
	protected ModelAndView handleRequestInternal(HttpServletRequest request, HttpServletResponse response) throws Exception {
		Language language = (Language) request.getAttribute(BlogLanguageMethodArgumentResolver.BLOG_LANGUAGE_ATTRIBUTE);
		if (language == null) {
			Blog blog = blogService.getBlogById(Blog.DEFAULT_ID);
			language = blog.getLanguage(blog.getDefaultLanguage());
		}
		...
		Page page = pageService.getPageByCode(variables.get("code"), language.getLanguage());
		if (page == null) {
			page = pageService.getPageByCode(variables.get("code"), language.getBlog().getDefaultLanguage());
		}
		if (page == null) {
			throw new HttpNotFoundException();
		}
